...
void AV_Player::BeginPlay()
{
	...
	if (APlayerController* PlayerController = Cast<APlayerController>(Controller))
	{
		...
		{
			if (m_CameraMovementMappingContext) {
				Subsystem->AddMappingContext(m_CameraMovementMappingContext, 1);
			}
			else {
				GEngine->AddOnScreenDebugMessage(-1, 5, FColor::Red, FString::Printf(TEXT("No Camera Movement Mapping Context")));
			}
...
void AV_Player::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(m_LookAction, ETriggerEvent::Triggered, this, &AV_Player::Look);
